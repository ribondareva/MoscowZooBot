...
class Order(Base):
    ...
    # Добавляем уникальное ограничение для столбца name
    __table_args__ = (
        UniqueConstraint('name', name='uq_order_name'),
    )
...
def add_order_with_on_conflict(session, name, class_id):
    ...
    session.execute(
        text("INSERT INTO orders (name, class_id) VALUES (:name, :class_id) ON CONFLICT (name) DO NOTHING"),
...
def add_family_with_on_conflict(session, name, order_id):
    ...
    session.execute(
        text("INSERT INTO families (name, order_id) VALUES (:name, :order_id) ON CONFLICT (name, order_id) DO NOTHING"),
    ...
    for item in data:
        try:
            ...
            if any(key not in item or not item[key].strip() for key in required_keys):
                print(f"Пропускаем {item.get('Название животного', 'Без названия')} — неполная классификация.")
                continue
            ...
            with session.begin():
                ...
                if not animal_class:
                    raise ValueError(f"Класс '{item['Класс']}' не был создан или найден.")
                ...
                if not order:
                    raise ValueError(f"Отряд '{item['Отряд']}' не был создан или найден.")
                ...
                if not family:
                    raise ValueError(f"Семейство '{item['Семейство']}' не было создано или найдено.")

                # Получаем или создаем род
                genus, _ = get_or_create(session, Genus, name=item['Род'], family_id=family.id)

                # Проверяем существование животного
                animal = session.query(Animal).filter_by(name=item['Название животного']).first()
                if not animal:
                    animal = Animal(
            ...
            print(f"Ошибка при обработке записи {item.get('Название животного', 'Без названия')}: {e}")